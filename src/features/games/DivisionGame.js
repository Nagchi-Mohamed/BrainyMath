...
export default function DivisionGame() {
  ...
  const { score, incrementScore, resetScore } = useScore('division');
  
  // Adaptive difficulty based on performance
  const difficultyLevel = Math.min(8, Math.floor(score / 5) + 1);

  const generateProblem = useCallback(() => {
    // Generate division problems with integer results
    const b = Math.floor(Math.random() * difficultyLevel) + 1;
    const answer = Math.floor(Math.random() * difficultyLevel) + 1;
    const a = b * answer; // Ensures a clean division
    return { a, b, answer };
  }, [difficultyLevel]);
